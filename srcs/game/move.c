...
/*   Updated: 2023/10/17 20:02:21 by cprojean         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_up(t_data *cube)
{
	...
	if (is_wall(cube, cube->j1.posx + cube->j1.cam_dx, \
		cube->j1.posy + cube->j1.cam_dy) == 0)
	{
...
void	move_right(t_data *cube)
{
	...
	if (is_wall(cube, cube->j1.posx + cam_dx, \
		cube->j1.posy + cam_dy) == 0)
	{
		cube->j1.cam_dx = cam_dx;
		cube->j1.cam_dy = cam_dy;
		cube->j1.posy += cube->j1.cam_dy;
